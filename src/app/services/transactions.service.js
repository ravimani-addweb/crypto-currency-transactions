...
class Transactions {
    ...
    calculateMosaicsFee(multiplier, mosaics, attachedMosaics) {
        ...
        for (let i = 0; i < attachedMosaics.length; i++) {
            ...
            totalFee += this._feeFactor * Math.max(1, fee - supplyRelatedAdjustment);
        }
        return totalFee;
    }
    ...
    _multisigWrapper(senderPublicKey, innerEntity, due) {
        ...
        let custom = {
            'fee': (this._baseFee * this._feeFactor) * 1000000,
    ...
    _constructTransfer(senderPublicKey, recipientCompressedKey, amount, message, due, mosaics, mosaicsFee) {
        ...
        let msgFee = message.payload.length ? this.calculateMessageFee(message) : 0;
        let fee = mosaics ? mosaicsFee : this._feeFactor * helpers.calcMinFee(amount / 1000000);
    ...
    /**
     * Calculate the fee needed for a given `message` length.
     *
     * @param {string} message - The message for which to retrieve the fee.
     *
     * @return {float} - The needed message fee.
     */
    calculateMessageFee(message) {
        if (!message.payload || !message.payload.length)
            return 0.00;

        return this._feeFactor * (Math.floor((message.payload.length / 2) / 32) + 1);
    }
