...
class DataBridge {
    ...
    constructor(AppConstants, Alert, NetworkRequests, Wallet, $timeout, $filter) {
        ...
        /**
         * Store the FIAT currencies exchange rates.
         * 
         * This will store the price for INR, EUR and
         * GBP so that we can convert from USD.
         * 
         * @type {object|undefined}
         */
    ...
    openConnection(connector) {
        ...
        connector.connect(() => {
            ...
            // Gets btc price
            this._NetworkRequests.getBtcEuroPrice().then((data) => {
                    this._$timeout(() => {
                        this.btcEuroPrice = data;
                    });
                },
                (err) => {
                    this._$timeout(() => {
                        this._Alert.errorGetBtcPrice();
                        this.btcEuroPrice = undefined;
                    });
                });

            // Gets fiat currencies exchange rates
            this._NetworkRequests.getFiatRates().then((data) => {
                    this._$timeout(() => {
                        this.fiatRates = data;
                    });
                },
            ...
            let mosaicCallback = (d, address) => {
                this._$timeout(() => {
                    ...
                    this._NetworkRequests.getStockData(d.mosaicId.name).then((apiData) => {
                            d.price = d.mosaicId.name == 'coin' ? '0.04' : apiData.price;
                            ...
                            d.price = isNaN(d.price) ? 0.00 : d.price;
                        },
                        (err) => {
                            d.price = 0.00;
                            d.listType = '';
                    ...
                    //console.log('API data: '); console.log(d);
                    //d.price = this.stockPrice;
