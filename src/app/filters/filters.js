...
let fmtLevyFee = ['fmtSupplyFilter', function(fmtSupplyFilter) {
    return function fmtLevyFee(mosaic, multiplier, levy, mosaics, div) {
        if (mosaic === undefined || mosaics === undefined) return mosaic;
        if (levy === undefined || levy.type === undefined) return undefined;
        if (div === undefined) div = 0;
        let levyValue;
        if (levy.type === 1) {
            levyValue = levy.fee;
        } else {
            // Note, multiplier is in micro NEM
            levyValue = (multiplier / 1000000) * (mosaic.quantity*Math.pow(10, div)) * levy.fee / 10000;
        }
...
let currencyFormat = function() {
    return function(number, div) {
        if (undefined == div) div = 2;
        if (undefined === number) {
            number = 0;
            return number.toFixed(div);
        }
        return parseFloat(number).toFixed(div).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
    }
}
