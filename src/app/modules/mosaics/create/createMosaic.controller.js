...
class createMosaicCtrl {
    ...
    send() {
        ...
        let unsafe = this.formData.mosaicDescription;
        let sanitized = unsafe && unsafe.length ? unsafe.replace(/</g, "&lt;").replace(/>/g, "&gt;") : "";

        // SANITIZE MOSAIC DESCRIPTION
        this.formData.mosaicDescription = sanitized;

        // sanitizing may break the 512 characters limit
        if(!this.mosaicDescriptionIsValid(this.formData.mosaicDescription)) {
            this._Alert.invalidMosaicDescription();
            return;
        }
